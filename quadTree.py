...
class QuadTree(object):
    ...
    def __init__(self, items, window, depth=8, bounding_rect=None, draw=False):
        """Creates a quad-tree.
        ...
        @param bounding_rect:
            ...
            internal use only.
        """
        # If the quad tree lines are being drawn or not
        #self.draw = draw

        if(draw):
                pygame.draw.rect(window, "gray", bounding_rect, 1)
        ...
        if nw_items:
            self.nw = QuadTree(nw_items, window, depth, (bounding_rect.left, bounding_rect.top, cx, cy), draw)

        if ne_items:
            self.ne = QuadTree(ne_items, window, depth, (cx, bounding_rect.top, bounding_rect.right, cy), draw)

        if se_items:
            self.se = QuadTree(se_items, window, depth, (cx, cy, bounding_rect.right, bounding_rect.bottom), draw)

        if sw_items:
            self.sw = QuadTree(sw_items, window, depth, (bounding_rect.left, cy, cx, bounding_rect.bottom), draw)
...
if __name__ == "__main__":
    ...
    for i in range(150):
        flock.append(
            Boid(uniform(width / 4, 3 * width / 4), uniform(height / 4, 3 * height / 4))
        )
